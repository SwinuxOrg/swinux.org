+++
title = "About Swinux"
showFullContent = false
readingTime = false
hideComments = false
+++

Swinux - The Swinburne Linux Hardware & Code Club - is a club established in 2024 and is dedicated to facilitating a space for everyone who is curious or interested about Linux, Software or Hardware to network and learn new skills. 

Everyone is welcomed to join, no matter what level you feel you are at in regards to tech. We want to see **your** passion for **everything** tech related. 

If you've got something cool to share, whether it's a project, new software or hardware, we can faciliate that through the club via blogs, pre-recorded videos or in person lectures. Feel free to contact the committee with your ideas, we want to see them!

We also host in-person meetups, where we chat about tech, share knowlege and work on our assignments. You can find out about them when we announce them on our socials.

Our socials are below:
- Instagram: [@swinuxclub](https://www.instagram.com/swinuxclub/)

- Discord: [Swinburne Linux Hardware & Code Club](https://discord.gg/ZfCW8DhnhA)

- Club Page: [Swinburne Linux, Hardware and Code Club](https://campus.hellorubric.com/?s=12119)

- Email: contact@swinux.org


This site is hosted on Cloudflare and built using Hugo. The website repository is public, and can be found [here](https://github.com/SwinuxOrg/swinux.org).
